...
\begin{abstract}
 The matrix model on a plane-wave background is discussed in the classical regime.  We propose a geometry for the trivial vacuum motivated by a specific limit that arises from the M-theory description where the data of M-theory can be mapped to an electrostatic problem in three dimensions. Furthermore, we investigate the trivial vacuum of the BMN model and give a gravity description in terms of D0-branes forming and probing this geometry. Using a single D0-brane as  probe, we find a repulsive behavior approaching the core of this geometry. We comment on implications believing also that it lies in the reach of Monte-Carlo simulations.    
\end{abstract}
...
Furthermore, we remind  that the compactified M-theory along the circle with $R_s$ radius, where $R_s=\sqrt{\alpha'}g_s$ becomes ten dimensional in the $R_s\to 0$ limit. In fact this is believed to be the type IIA string theory and actually it is used as the definition of it. Moreover, in this limit the low energy sector which consists of D0-branes decouples. One way to realize this is that M-theory contains also membranes and these in general can wrap the compactified circle, however in the boosted frame these membranes  become non-local while the longitudinal modes remain local. From the type IIA point of view the only local elements are the D0-branes. Thus, this sector decouples from the rest of the system. The $N-$th Kaluza-Klein mode corresponds to $N$ units of momentum $\frac{1}{R_s}$ in the compact dimension, and in this case, the $U(1)$ sector is the R-R one form of type IIA. Hence, we interpret D0-charge as eleven-dimensional momentum. In this way the arbitrary Kaluza-Klein modes on the compactified circle we described above are massless modes and are materialized as supegravitons. Being themselves massless in the eleven-dimensional spacetime, they appear in the ten-dimensional spacetime as pointlike D0-branes.  Having sketch  the connection of D0-branes with supergravitons (of eleven-dimensional M-theory), one might ask what about the rest of the objects like  membrane or fivebrane? As it is nicely explained in \cite{Sen1998}, there is no need to add extra degrees of freedom in the action to describe also these objects, but rather they can be described via D0-branes and T-dualities. 

The procedure of boosting, relates the non-relativistic ten dimensional theory of many D0-branes (large $N$) with a particular frame of the M-theory, the so-called infinite momentum frame (IMF). Then the conjecture is that the dynamics of the D0-branes are in one-to-one correspondence with the Hermitian matrices of the $(0+1)-$dimensional Yang-Mills theory  and they describe all the contents of M-theory in this particular IMF frame\footnote{Note however that the M5-brane of M-theory can not be easily realized from the Matrix model \cite{Berenstein2002}.}. A graphical representation of the above is given in figure \ref{duality}. The previous statement holds when $N$ is large. Further investigations have proposed that this should be true even at finite $N$ \cite{Susskind1997}. However since the equivalence principle breaks down for the finite $N$ version of the theory \cite{Taylor2000a}, \cite{Kabat1998} it is quite unlikely that it can be related to a smooth theory of Einstein-Hilbert gravity. On the other hand, this might not be a real problem since in the spirit of holography, finite $N$ corrections seem to correspond to quantum corrections, and from many different approaches quantum spacetime is believed to be discrete. Thus, it is more likely that the finite $N$ version of the model would correspond to a (quantum corrected) gravity theory on a non-commutative space. 

 Another realization of this duality which might be seen as evidence  is the following. Firstly, there is lack of a concrete microscopic description of M-theory, and since the contents of M-theory are supermembranes, the UV completion of this theory has to be regularized via these membranes. This procedure \cite{Nicolai1988} results in a Hamiltonian containing bosonic ($X$'s) and fermionic ($\Psi$'s) terms
...
\ee 
In the same Hamiltonian, one can arrive by the dimensional reduction of the $U(N)$ 10D-SYM theory. Also, since the strong coupling limit of type IIA string theory is supposed to be M-theory, one should also arrive in \eqref{Hamiltonian} by using type IIA terminology. Indeed, this is the case for a large number of D0-branes at low energy and weak coupling \cite{Banks1997}, \cite{TaylorIV1999}. 
Thus, we see that one can reach this particular Hamiltonian \eqref{Hamiltonian} by three different approaches, M-theory, type IIA string theory and SYM theory, and this strongly suggests that there must be an underlying relation among them. This was one of the motivations of Banks, Fischler, Shenker, Susskind to propose their conjecture \cite{Banks1997}.

It was only later when another proposal \cite{Berenstein2002} (BMN matrix model), realized also \cite{Banks1997} as a special case of a one parameter ($\mu$) family of maximally supersymmetric (32 supercharges) matrix models, whose Hamiltonian is a deformation of \eqref{Hamiltonian} by
...
\begin{itemize}
	...
	\item $X^a=0~,a=4,\cdots,9$ and $X^i=\frac{\mu}{3R}J^i~,i=1,2,3$, with $J^i$ being the generators of $SU(2)$ satisfying $[J_a,J_b]=i\epsilon_{abc}J_c$ and corresponds to the non-trivial vacuum. 
\end{itemize} 
...
\be 
ds^2_{11}=e^{-\frac{2\phi}{3}}ds_{10}^2+e^{\frac{4\phi}{3}}\left(dx^{10}-C_\alpha dx^\alpha\right)^2.
\ee
...
Having set up the playground, now we turn to calculate the force. When we do this, we have in mind a situation when we naively place a D0-brane probe in the spacetime and ask for its dynamical behaviour. To this end, we need to write down the DBI action for the D0-brane by choosing the gauge where the parameter of the worldline of the D0 coincides with the time component of the spacetime. In a general background, it is given by 
\be\label{bosonicpart}
S_{D_0}=-T_0\int dt e^{-\phi}\sqrt{-g_{\mu\nu}\frac{dx^\mu }{dt}\frac{dx^\nu}{dt}}+T_0\int dt C_0,
\ee
where $T_0=\frac{M_s}{g_s}$ is the tension of the D0-brane and the expression inside the square root is the pull-back of the spacetime metric on the worldline of the D0-brane. We remind that a Dp-brane naturally couples with a p+1 form, thus the only form that couples (naturally) to a single D0-brane is the one-form \footnote{We could of course consider a three-form flux and via this procedure the D0-brane would couple to also a three form, giving rise to a Myers effect. This would change the output of the discussion but we change in the first place the background by the introduction of the three-form.} potential $C_0$.  Then, the action reads as 
\be 
...
\ee
Moreover, after taking the non-relativistic limit we expand the action up to $\dot{x}^2$ and $Q^2$ terms leading to
\be 
...
\ee    
where $2\pi\alpha'\mathcal{F}_2=2\pi\alpha'F_2-B_2$. As in the case of the D0-brane, we have also chosen the static gauge here, which corresponds to identify the worldvolume dimensions with the three dimensional part of the subspace of the ten-dimensional spacetime. We can choose $t,\theta, \phi$ coordinates parametrizing the $S^2$. The radius of $S^2$ is given by $r$, while the angles have the usual ranges $\theta\in [0,\pi]$, $\phi\in[0,2\pi]$. For a compact D2-brane of arbitrary topology the total  charge $q$ is quantized and it is given as  
\be 
\int_{S^2}F_2=2\pi q ~~~\longrightarrow~~~ F_2=\frac{1}{2}q\sin\theta d\theta\wedge d\phi.
\ee 
We note that the integer $q$ is also related to the first Chern class of the U(1) bundle at a fixed point in time \cite{Figueroa-OFarrill2000}, \cite{Stanciu2000} and moreover, it acts as a source for the R-R vector field and the interpretation of it is the number of D0-branes that are bound on the surface of the D2-brane. 
...
\be 
r_i=\frac{4\pi\alpha'q_i\mu}{3},~~i=1,\cdots, k,
\ee   
...
\ee
In fact, we see that it is independent of the configuration of the geometry, for example independent of the warp factor $e^\frac{2\phi}{3}$, but it does depend on its own charge $q$ and the flux parameter $\mu$, in accordance with the D0-brane case. We also see that the force becomes repulsive for $r\in (0,\pi q\alpha'\mu)$. This results to an interpolation from a strong, attractive force to a slightly repulsive force after going through the minimum of the potential.  In the minimum, we have a D2-brane freely sitting on a position in spacetime. So in this way, we can see that we probe and reveal different parts of the geometry since this picture in the D0-probe was absent. 


\section{BMN vacua}\label{Vacua}
In this section, after discussing the matrix model vacua and how one can think this as a electrostatic configuration problem following \cite{Lin2006}, we describe the gravity duals.  For complementary material regarding this treatment see \cite{Lozano2017}, \cite{Lin2004a}, \cite{Costa2015}. Furthermore, we calculate the force acting on a D0-brane from the geometry.
...
\end{eqnarray}
 The function $V(z,\rho)$ appearing in \eqref{IIAmetric} can be interpreted as a potential that sources different brane (NS5, D2, D0) configurations, and depending on the specific form of this function one realizes different gravity solutions, which correspond to different vacua in the matrix model side.  We are interested in the bosonic sector of BMN because in that case we can write a metric ansatz that respects the symmetries. In the BMN case, we see from \eqref{HamiltonianBMN} that these symmetries are $R\times SO(3)\times SO(6)$. The first of these implies that there is a Killing vector associated to translations of the time coordinate. The second, implies that we can imagine a sector of the spacetime to consist of a $S^2$ sphere\footnote{Actually it is more appropriate to say that we have non-commutative sphere since the coordinates that are assigned to $SU(2)$ generators do not commute. } while the third the existence of a $S^5$ sphere, on which the bosonic generators act. Thus one can analyse solutions in eleven dimensions by using only three variables, $x_1,x_2,y$ which satisfy the Toda equation in three dimensions \cite{Lin2004},\cite{Lin2006}
\be \label{toda}
\left(\p_{x_1}^2+\p_{x_2}^2\right)V+\p_y^2V=0.
\ee
Here $y$ is used as a combination of the radii of $S^2$ and $S^5$ respectively, $y=r_2r_5^2$. Exploiting the translational invariance under one coordinate ($x_1$), and after changing variables $(x_2,y)\mapsto(\rho,z)$ one can recast this equation to the Laplace equation in a three dimensional space \cite{Lin2004}
\be \label{Laplace}
\frac{1}{\rho}\p_\rho\left(\rho\p_\rho V\right)+\p_z^2V=0.
\ee
We note that this three-dimensional space is not a sub-part of the ten dimensional one but rather maps to an electrostatic problem in a three dimensional space, where all gravity solutions are classified by $\rho,z$. Here, both $\rho$ and $z$ are radial coordinates, and hence one can actually think of them as two different scales of the theory. The former captures  the size of fivebranes and the latter the positions of the conducting disks, which is actually being set by the five-branes.  
...
In the case of the BMN model, the asymptotic expansion is decomposed in two parts which the first of these represents the electrostatic potential and the second one the specific electrostatic configuration imposed by the boundary conditions. In other words, this means that one can specify the solution but not necessary to which vacuum corresponds, since in principle there will be vacua with the same boundary conditions. The first non-trivial potential is \cite{Maldacena2003}
\be \label{dipole}
...
\ee
where $i$ runs over the various disks, $\mathcal{Q}_i$ is the charge of the $i$-th disk and $N$ is the total number of D0-branes.\footnote{Here we make contact with the introductory remarks, where we stated that all the contents can be reconstructed from D0-branes.} The number of D0-branes can be extracted from the particular configurations of the disks. Suppose for example that there is a finite set of conducting disks at positions $z_i=\frac{\pi}{2}N_5^{i}$, with radii $R_i$ and charge $\mathcal{Q}_i=\frac{\pi^2}{8}N_2^{i}$. Then the disks are sourced by the NS5-branes in the background, with the charge $\mathcal{Q}_i$ being associated to the D2-brane charges. In this sense, one can write the number of D0-branes as 
\be 
N=\sum_{i}\left(\sum_{j<i}N_5^{j}\right)N_2^{i}.
\ee    
Furthermore,  $N_2^{j}$ is related to the copies of dimension $N_5^{i}=(2j+1)$ irreducible representations, giving a geometric realization of the situation of the matrix model.
These, correspond to concentric spheres and due to the non-commutativity are actually concentric  fuzzy spheres. We will not analyse these fuzzy geometries in this paper, since our interest is for the trivial vacuum.

\noindent
To get the trivial vacuum metric one has to perform a two steps procedure. The first step is to plug in  \eqref{dipole} into the metric \eqref{IIAmetric} and expand at large $\rho$ and $z$. Then change coordinates  as
\be \label{coordchange}
r^2=16\left(\rho^2+z^2\right),
\ee
 and in the large $r$ limit the metric approximates the near horizon geometry of $N$ coincident D0-branes \cite{Lin2006}
\be \label{larger}
...
However the large $r$ region is a bit subtle. Let us try to clarify some possible issues. The supergravity description is valid when $g_{\text{eff}}=\frac{\lambda}{\mu^3}\gg 1$ while for $g_{\text{eff}}\ll 1$ we can trust perturbative calculations in matrix model. The first case corresponds to a region between the black hole (i.e near the core of the geometry in figure \ref{bulk}) and  $r\sim\lambda^{1/3}$, while the second case is near the boundary. However, near the boundary the gravity solution is not valid since the curvature is high (see next subsection).   Hence we are in a region when $r$ is large but nevertheless not near the boundary. It is large with respect of the core of the geometry and a good estimate would be $r\lesssim \lambda^{1/3}$. Here we can imagine the following situation, we suppose that we want to study the system in a specific energy scale (i.e fixed $\lambda$), where we can trust the supergravity solution. This amounts to 
\be 
r\gg 1~~\text{\underline{and}}~~g_{\text{eff}}=\frac{\lambda}{\mu^3}\gg1,~~\lambda=\text{fixed},~\Rightarrow \mu\to 0.
\ee 
Thus, to leading order we recover the case of BFSS model and this is what the metric \eqref{larger} tells us about.

 
\subsection{Trivial vacuum}
From the gauge theory side, it seems that the easiest classical vacuum, is the one that all matrices vanish trivially, i.e $X^i=0=X^a$, with $ i=1,2,3~~,~~a=4,\cdots,9$. This corresponds to the situation where we have  $N$ copies of the trivial ($j=0$) representation of $SU(2)$, and in the electrostatic description this translates to $N_2=N$ and $N_5=1$, i.e a single disk at a distance close to (of order one unit) the infinite conducting disk at $z=0$ (see figure \ref{rzplane}).  This vacuum is believed to correspond to a single fivebrane in the gravity side \cite{Maldacena2003}, and the gravitational approximation is believed to not be very good near the fivebrane  \cite{Berenstein2002}, \cite{Lin2006}. Nevertheless, we perform an expansion in the metric in the large $\rho$ limit while keeping $z$ of order one. Thus the trivial vacuum limit corresponds to 
\be 
r^2=16\left(\rho^2+z^2\right)\gg1,~~\rho\gg1,~~~z\sim\mathcal{O}(1).
\ee 
\subsubsection{Zero temperature}
We will start applying this to zero temperature first and then move to finite temperatures in the next subsection. The recipe indicates that we firstly plug \eqref{dipole} into \eqref{IIAmetric} and then taking the trivial vacuum, we get a metric of the form 
\begin{align} \label{trivialvac}
ds^2&= -H(r)^{-\frac{1}{2}}dt^2+16H(r)^{\frac{1}{2}}\left(dz^2+z^2d\Omega_2^2\right)+H(r)^{\frac{1}{2}}\left(dr^2+r^2d\Omega_5^2\right),
\\ H(r)&:= \frac{256\cdot 15\mathcal{P}}{r^7},\\
...
\\
\label{oneform}A_1(r)&=-\frac{r^7}{2^{11}15\mathcal{P}}-\frac{7z^2r^5}{2^815\mathcal{P}}-\frac{7z^4r^3}{192\mathcal{P}}+\frac{r^2}{10}-\frac{7z^6r}{24\mathcal{P}}-\frac{12z^2}{5},
\end{align}
\noindent
We have kept only leading terms both in the expression of dilaton and the one form. We have kept explicitly the $z$ dependence but one should remember that is of order one. The next correction to the dilaton is of order $\mathcal{O}(r^{-29/4})$ and in the large $r$ limit we neglect it. In the same limit, the contribution of the R-R one form is significant. Furthermore, we see that in this limit and for same energy scale (or the same radius $r$), the effective curvature of the five-sphere, which in string units is the inverse of $\frac{\mathcal{R}_5^2}{\alpha'}=H(r)^{\frac{1}{2}}r^2$, is much smaller than the one of the two-sphere, which is the inverse of $\frac{\mathcal{R}_2^2}{\alpha'}=H(r)^{\frac{1}{2}}z^2$ . This means that curvature corrections coming from the two-sphere part of the metric may become important when compared with the ones of the five-sphere \cite{Lin2006} \footnote{Both $r$ and $z$ here are energy scales, and this condition reflects the fact that we push the regime of validity of the metric to its boundary. In \cite{Maldacena2018}, and based on \cite{Polchinski1999} was argued that the breaking of Einstein supergravity starts at scales $\lambda^{1/3}$.}.   We also note that the validity of the supergravity solution is when the dilaton is not too large and when the effective curvature of the five-sphere is not too big. Recalling $\mathcal{P}\sim N$ this translates to the range
\be \label{validsugra}
...
\noindent
Lastly, before moving to finite temperature, it worths mentioning that after a coordinate rescalling and transformation, namely
\be 
z\to\frac{\zeta}{4}~~,~~R\to \frac{2}{5}\frac{r_0^{5/2}}{r^{5/2}},~~r_0^7:= 256\cdot 15\mathcal{P},~~t\to r_0t
\ee  
the metric  \eqref{trivialvac} becomes 
\be 
...
\ee 
Therefore, the near horizon limit of the vacuum solution is conformaly equivalent to $AdS_2\times S^5\times S^2$ if we keep $r_0$ fixed which means to keep $\mathcal{P}$ fixed. It is obvious (from the limits of validity) that the "size" of the $AdS_2$ part of the metric is much bigger than the $S^2, S^5$, since the trivial vacuum corresponds to large $r^2=16(\rho^2+z^2)$, with $\rho$ large and $z$ of order one. We note however that in the range of validity the $S^5$ part is bigger than the $S^2$ part of the metric. 
\begin{figure}
\centering 
...
\caption{A pictorial representation of the regime of gravity and the regime of matrix model. In this way one can approximate different regimes with the help of the radial parameter $r$, the deformation parameter $\mu$ and the effective coupling $g_{\text{eff}}=\frac{\lambda}{\mu^3}$.}
\label{bulk}
\end{figure}


\subsubsection{Force at finite temperature}\label{sectionfiniteT}
At finite temperature the BMN model has $1/2$- BPS states \cite{Berenstein2002}, \cite{Dasgupta2002} and we should expect the force to be present even if we consider fermions. Eventually, also we would like to make some contact with the quantum mechanics side. The quantum mechanics however is a subsector of  the SYM theory. So
...
\be 
\frac{R^2_{S^5}}{\alpha'}=4\pi\rho_0=4\pi\left(\frac{\lambda}{2\mu^3}\right)^{\frac{1}{4}}.
\ee  
...
\end{align}
and the horizon is located at $r=r_0$, with $r_0$ representing the thermal energy scale.  The horizon is dictated only from the radius $r$, but we have implicitly assigned  $r^7:=r_5^5\cdot z^2$, where $r_5, z$ the radii of the 5-sphere and 2-sphere respectively. In other words, we have merged the two scales into one. We can proceed even more and pick a scale for the $S^2$ part such that we measure everything at this fixed scale. We will choose this to be $z=\frac{\pi}{2}r_2$ and this was already implemented in the expressions of \eqref{finiteT} by setting $r_2=1$, but not in the metric itself.  This choice reflects the trivial vacuum limit discussed above and will be used when we calculate the force.

\noindent
 Demanding the curvature of the spheres to be not too large  we get the condition 
\begin{align}
r&\lesssim \lambda^{1/3} ~~~~~~~\text{both from $S^5$ and $S^2$}.
\end{align}
 There is also one more constraint coming by demanding that the dilaton is not too large on the horizon which results in 
\be 
\lambda \beta^3\lesssim N^{10/7},
\ee
and for sufficiently large $N$ with respect to the combination $\lambda\beta^3$ the last condition is automatically satisfied.

\noindent
From this metric one can extract the temperature via the surface gravity 
\be 
T=\frac{7 r_0^{5/2}}{4\pi \sqrt{\lambda d_0}}
\ee 

\noindent 
The calculation of the force uses a variation of section \ref{classicalforce},
\be \label{potentialfiniteT}
V_{D_0}(r)=T_0\left[ e^{-\phi}H(r)^{-\frac{1}{4}}f(r)^\frac{1}{2}-A_1(r)\right].
\ee 
 with the data of the finite temperature gravity solution \eqref{finiteT}. We expect qualitatively similar behaviour as in the zero temperature system because of BPS states, and evaluating indeed we see the behavior in figure \ref{F-V-T}. Here we show the dependence of everything in terms of $\lambda$ to make conduct with the existing literature. One, can understand that there is also a hidden  $\mu$ dependence  if we use the dimensionless coupling $g_{\text{eff}}=\frac{\lambda}{\mu^3}$.
\begin{figure}[h]
	\centering 
	\includegraphics[scale=0.48]{FiniteTForce}
	\includegraphics[scale=0.48]{FiniteTpot}
	\caption{Finite temperature force (left) and potential (right) with respect to the radial coordinate $r$ (or equivalently temperature $T$). We have set for the horizon $r_0=1$ and $g_{YM}=1$. }
\label{F-V-T}
\end{figure}

What is different here  in contrast to the BFSS case, is the fact that repulsive phenomena can occur even classically and far away from the horizon if $\lambda$ is sufficiently large. This is also the regime where the gravity description is valid, and we can understand this repulsive phenomenon as the R-R charge of the D0-branes. Moreover,  it is known for the BMN model that there might be a tunneling effect between "nearby" vacua \cite{Lin2006a}, where the probability to tunnel is given by the exponential $e^{-\frac{N}{\lambda}}$ since it is mediated by instantons. $N$ is the number of the degrees of freedom (i.e D0-branes) and $\lambda$ the t'Hooft coupling. As a result in the regime we are interested in, the tunneling between different vacua is exponentially suppressed. 

The main lesson is that the behaviour of the D0-probe reported here is at least \textit{qualitatively} similar to quantum corrected black 0-brane geometries \cite{Hyakutake2014} which were verified by matrix simulations  \cite{Masanori2017}. The analysis performed here is purely classical and furthermore is $\lambda$ independent. This can be seen from the fact that the curves in figure \ref{F-V-T} join going towards $r\to0$ and the steep regions are outside of the regime of validity. One can imagine fixing $\lambda$ and taking $\mu\to 0$ such that the condition of supergravity $g_{\text{err}}\gg 1$ is satisfied. Then, in principle this behaviour should be visible in matrix simulations along the lines of  \cite{Masanori2017}.



We also note that the geometry which is believed to correspond to BFSS model is recovered when we take $\mu\to 0$. At first sight this might not be obvious but we observe the following. We always want the gravity description to be valid which results in $g_{\text{eff}}\gg1$ and we also want $r\gg1 $ such that we are away from the singularity. If we fix $\lambda$ which amounts to fix the energy scale of the system, we have that 
\be 
g_{\text{eff}}=\frac{\lambda}{\mu^3}\gg1,~~\lambda=\text{fixed}~~\Rightarrow~~ \mu\to0.
\ee 
Then for fixed $\lambda\gg \mu^3$ with $\mu\to 0$, the dominant contribution in the R-R one-form comes from the $r^7$ term for large $r$. This is also the case for BFSS model where the R-R form is given in terms of $r^7$ \cite{Maldacena2018}, \cite{Itzhaki1998}. As we saw, the same limit is what constructs the metric \eqref{larger} to leading order, which is the metric for the gravity dual of the BFSS.  


\section{Synopsis}\label{synopsis}


We have demonstrated gravitational behaviors of D-brane probes (D0-and D2-branes) for symmetry reduced pp-wave backgrounds. In particular for the D0-probe we have also studied its behavior for a geometry which corresponds to the trivial vacuum of BMN. We have seen a repulsive behavior as we approach the core in the extremal case and the horizon in the non-extremal case. This most likely indicates that there is a mechanism that gives rise to repulsive forces as one approaches the core \cite{Peet2000}. We have demonstrated, that this mechanism can occur both at classical level and also far away from the core for the BMN model.  One possible way to understand this intuitively is because as an observer (here a D0-brane) approaches the core, firstly meets the $S^5$ part of the horizon surrounding the core and then the $S^2$. This particular probe measures everything in terms of the $SU(2)$ part of the geometry since we have fixed the scale $z$ that controls the size of $S^2$. This is a choice based on observations made in subsection \ref{D0reduced}. There we have showed that a single D0-brane is agnostic for the existence of $SU(2)$ while in the next subsection we have demonstrated that a D2-brane is aware for the $SU(2)$ part.   Now the charges of $S^2$ and $S^5$ consisting from D0-branes carry one forms with which the probe brane couples naturally. However, having the same charge the one form $A_1(r)$ causes repulsion. Moreover, this is something that classically does not appear in the BFSS model but rather when one considers quantum corrections \cite{Hyakutake2014}. It is natural to expect that this deformed geometry of BMN model modifies slightly the behaviour of the D0-probe even classically since they are in principle different backgrounds. 

 Nevertheless, the regime where we can trust supergravity depends on $g_{\text{eff}}$ and hence for large $g_{\text{eff}}$ one is already far away from the singularity. Sufficiently far away the D0-probe will be at the minimum of the potential. This follows from the analysis done in the main text and from the principle of least action and it holds only in the trivial vacuum geometry. Further studies need to be carried out to understand the situation of the non-trivial vacua which from geometric point of view are significantly more complicated.  The limit which we took to construct this geometry is also realized in simulations (\textcolor{red}{Cite to be appeared?}). We are referring to the  deformation of the geometry, as in the main text several times was emphasized  that the horizon of this geometry is of $S^5\times S^2$ topology with the size of $S^5$ being  bigger than that of $S^2$. However, one significant caveat of the construction is that we did not check explicitly that the vacuum geometries \eqref{trivialvac} and \eqref{finiteT} are solutions of Einstein's equations, but it is rather based on the spirit of \cite{Lin2006}.
 
 
  Understanding these bulk descriptions will be beneficial to extract meaningful physics when one also compares with the matrix model side both in analytic, and nowadays,  in numerical results.      
