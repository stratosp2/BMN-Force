...
We have demonstrated gravitational behaviors of D-brane probes (D0-and D2-branes) for symmetry reduced pp-wave backgrounds. In particular for the D0-probe we have also studied its behavior for geometry which corresponds to the trivial vacuum of BMN. We have seen a repulsive behavior as we approach the "naked" singularity in the extremal case and the horizon in the non-extremal case. This most likely indicates that there is a mechanism that gives rise to repulsive forces as one approaches either the singularity or the horizon. We have demonstrated, that this mechanism can occur both at classical level and far away from the singularity/horizon for the BMN model.  One possible way to understand this intuitively is because as an observer (here a D0-brane) approaches the core, firstly meets the $S^5$ part of the horizon surrounding the core and then the $B^3$ ball. This particular probe does not "see" the $SU(2)$ part of the geometry, but rather realizes a ball $B^3$. Now the charges of $B^3$ and $S^5$ consisting from D0-branes carry one forms with which  the probe brane couples naturally. However, having the same charge the one form $A_1(r)$ causes a repulsive phenomenon.  This is something that classically does not appear in the BFSS model but rather when one considers quantum corrections \cite{Hyakutake2014}.
